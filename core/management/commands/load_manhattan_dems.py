...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if filename:
            ...
            request = requests.get(settings.MANHATTAN_DEMOCRATS_URL)
            data = request.json()
    ...
    def parse_datetime(self, date_time, time_zone):
        ...
        Afterwhich, convert it to New York
        '''
        co_time = None
        
        dt = parser.parse(date_time).replace(tzinfo=pytz.timezone(time_zone)) if time_zone is not None else parser.parse(date_time)
        co_time = dt.astimezone(pytz.timezone("America/New_York")).replace(minute=dt.minute) if time_zone is not None else dt
    ...
    def store_organization(self):
        url = "http://www.manhattandemocrats.org"
        organization_type='democratic'
        title = "Manhattan Democrats"
